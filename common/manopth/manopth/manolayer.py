...
class ManoLayer(Module):
    ...
    def __init__(self,
        ...
        if th_v_template is not None:
            self.register_buffer('th_v_template',
                                 torch.Tensor(th_v_template.copy()).unsqueeze(0),
                                 )
        else:
    ...
    def forward(self,
        ...
        if th_trans is None or bool(torch.norm(th_trans) == 0):
            ...
            th_trans_mat = torch.zeros(th_jtr.shape[0], 4, 4).to(th_jtr.device)
            th_trans_mat[:, :3, 3] = th_trans
            th_results_global = th_results_global + th_trans_mat.unsqueeze(1)
