...
class MSDFOptimizer(torch.nn.Module):
    ...
    def _sample_mesh(self, surface_points_count=2048, vicinities_count=2048):
        ...
        surface, surface_normals, vicinities, vicinities_sdf = map(
            lambda x: torch.from_numpy(x).float(), (surface, surface_normals, vicinities, vicinities_sdf))
    ...
    def optimizer_step(self):
        ...
        surface_pred = msdf_at_point(surface, self.centers, self.scales, self.Vi, self.kernel_size)
        vicinities_pred = msdf_at_point(vicinities, self.centers, self.scales, self.Vi, self.kernel_size)

        surface_loss = surface_pred.abs().mean()
        vicinitties_loss = (vicinities_pred - vicinities_sdf).abs().mean()

        surface.requires_grad_(True)
        sdf = msdf_at_point(surface, self.centers, self.scales, self.Vi, self.kernel_size)
...
#                   output_path: str = 'output/msdf.pt',
#                   visualize: bool = True,
#                   resolution: int = 32,
