...
"""
Modified from Motion Diffusion Models (https://github.com/GuyTevet/motion-diffusion-model.git)
"""
...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps, beta_range=[0.0001, 0.02], scale_betas=1.):
    ...
    if schedule_name == "linear":
        ...
        beta_start = scale * beta_range[0]
        beta_end = scale * beta_range[1]
        return np.linspace(
...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output = model(x, self._scale_timesteps(t), cond=condition, **model_kwargs)
    ...
    def training_losses(self, model, data, **kwargs):
        ...
        model_output = model(x_t, self._scale_timesteps(ts), condition)
    ...
    def recon_loss(self, recon_contact, recon_cse, gt_contact, gt_cse):
        loss_contact = F.mse_loss(recon_contact, gt_contact, reduction='mean')
