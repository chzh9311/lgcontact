...
from common.model.diff.utils import timestep_embedding, ResBlock, SpatialTransformer, DualSpatialTransformer
from common.model.pointnet_module import Pointnet_feat_net
# from .scene_model import create_scene_model

from common.model.pointnet_module import PointNet2seg
...
class UNetModel(nn.Module):
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.obj_feat_net = PointNet2seg(**cfg.obj_encoder)
    ...
    def forward(self, x_t: torch.Tensor, ts: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        ...
        h = self.out_layers(h)
        h_c, h_cse = h.split([1, self.d_x - 1], dim=1)
        h_c = F.sigmoid(h_c)
        h = torch.cat([h_c, h_cse], dim=1)
    ...
    def condition(self, data: Dict) -> torch.Tensor:
        ...
        obj_feat = torch.cat([local_obj_feat, glob_obj_feat.repeat(1, 1, local_obj_feat.shape[2])], dim=1)
